...
def main():     #csv file contains my name and small(56) and large(28) data set i should use
    ...
    fileName = "Data/Ver_2_CS170_Fall_2021_SMALL_data__61.txt"
...
def leave_one_out_cross_validation(data, current_set_of_features, feature_to_add):      #current set of features is a list of the features we want to test
    ...
    for i in range(newData.shape[0]):           #goes through each instance(row) in data
        ...
        label_object_to_classify = newData[i][0]       #gets the label classifier in the first column          MAKE ALL UNWANTED COLUMNS EQUAL TO ZERO
