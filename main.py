...
def backwardElimination(data):
    ...
    for i in range(1, numOfFeatures):
        ...
        print(f"At level {i}, I removed feature {feature_to_remove_at_this_level} from the current set to achieve an accuracy of {least_so_far_accuracy}")
        if(timeElapsed > 7200):
            break
    return
...
def feature_search_demo(data):
    ...
    for i in range(1, numOfFeatures):
        ...
        if (timeElapsed > 7200) or (accDecreasing and highestAccuracyFound > best_so_far_accuracy):      #called before the next if statement so if algorithm notices accuracy decreasing, we run one more time 
